...
rd = lambda a,b: math.prod([int(s) for s in (a[b],a[b+GN+1],a[b+GN*2+2],a[b+GN*3+3])])
od = lambda a,b: math.prod([int(s) for s in (a[b+OFFS],a[b+GN-1+OFFS],a[b+GN*2-2+OFFS],a[b+GN*3-3+OFFS])])
facotr_return = lambda a,b: [int(s) for s in (a[b+OFFS],a[b+GN-1+OFFS],a[b+GN*2-2+OFFS],a[b+GN*3-3+OFFS])]
...
for y in range(0,GN*(GN-OFFS)+1,GN):
    for x in range(0,GN-OFFS+1):
        ...
        result.append(rd(gl,x+y))
        result.append(od(gl,x+y))
#        if max(result) != max_value:
#            max_value = max(result)
#            factor = facotr_return(gl, x+y)
print(f"{max(result)=:5,d}")
t2 = time.time()
elapse = (t2 - t1)*10**3
